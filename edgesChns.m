...
if (opts.cordchanel==1)
    [xs,ys] = meshgrid(1:size(I,1),1:size(I,2));
    xs = xs./size(I,1)*10000 + 10000;
    ys = ys./size(I,2)*10000 + 10000;
    xs = single(xs);
    ys = single(ys);
end
...
%   if(size(I,3)==1), cs='gray'; else cs='luv'; end; I=rgbConvert(I,cs);
  ...
  if (opts.cordchanel==1)
   cordshrink = imResample(xs,1/shrink); k=k+1; chns{k}=cordshrink;
   cordshrink = imResample(ys,1/shrink); k=k+1; chns{k}=cordshrink;
  end
  
  for i = 1:2, s=2^(i-1);
    if(s==shrink), I1=Ishrink; else I1=imResample(I(:,:,1),1/s); end
    I1 = convTri( I1(:,:,1), opts.grdSmooth );
