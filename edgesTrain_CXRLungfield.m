function model = edgesTrain_CXRLungfield( varargin )
...
dfs={'imWidth',32, 'gtWidth',16, 'nPos',2e5, 'nNeg',2e5, 'nImgs',2000, ...
  'nTrees',8, 'fracFtrs',0.25, 'minCount',4, 'minChild',8, ...
  'maxDepth',16, 'discretize','pca', 'nSamples',256, 'nClasses',2, ...
  'split','gini', 'nOrients',4, 'grdSmooth',0, 'chnSmooth',2, 'cordchanel',0,'pixelresolution',1,'subRegionID',0,...
  'simSmooth',8, 'normRad',4, 'shrink',2, 'nCells',5, 'rgbd',1,'autoContext',0, ...
  'stride',2, 'multiscale',0, 'sharpen',0, 'nTreesEval',4, ...
  'nThreads',4, 'nms',0, 'seed',5577795, 'useParfor',0, 'modelDir','models/', ...
  'modelFnm','model_CXR', 'bsdsDir','ImageData\JSRT Chest X-Ray\All247images\','trnGtDir','ImageData\JSRT Chest X-Ray\scratch\fold2\masks\'};
opts = getPrmDflt(varargin,dfs,1);%'seed',5577795;9575795
...
nChnsGrad=(opts.nOrients+1)*2; 
if(opts.rgbd==0), nChnsColor=2; end
if(opts.rgbd==1), nChnsColor=1; end
if(opts.rgbd==2), nChnsGrad=nChnsGrad*2; nChnsColor=nChnsColor+1; end
if(opts.cordchanel==1), nChnsColor = nChnsColor + 2; end
if(opts.autoContext==1), nChnsColor = nChnsColor+1; end
...
% stream=RandStream('mrg32k3a','Seed','shuffle');
...
% trnGtDir = 'D:\ImageData\JSRT Chest X-Ray\scratch\fold2\masks\';
trnGtDir = opts.trnGtDir;
...
%   I=imread([trnImgDir imgIds{i} '.' ext]);
%   siz=size(I);
%   if(rgbd), D=single(imread([trnDepDir imgIds{i} '.png']))/1e4; end
%   if(rgbd==1), I=D; elseif(rgbd==2), I=cat(3,single(I)/255,D); end
 
if(opts.autoContext~=1)
     if (rgbd==1)
          [ I, gt, ~, mask ] = getCXRgroundtruth( i,trnGtDir,opts.pixelresolution);
     elseif (rgbd==0)
          [ I, gt, ~ ,mask ] = getCXRgroundtruth2( i,trnGtDir,opts.pixelresolution);
     end
else
    [ I, gt, ~ ,mask ] = getCXRgroundtruth2_autoContext( i,trnGtDir,opts.pixelresolution);
end
  ...
  for j=1:nGt
    ...
    [y,x]=find(M.*B.*mask); k2=min(length(y),ceil(nPos/nImgs/nGt));
    rp=randperm(length(y),k2); y=y(rp); x=x(rp);
    xy=[xy; x y ones(k2,1)*j]; k1=k1+k2; %#ok<AGROW>
    [y,x]=find(~M.*B.*mask); k2=min(length(y),ceil(nNeg/nImgs/nGt));
    rp=randperm(length(y),k2); y=y(rp); x=x(rp);
    xy=[xy; x y ones(k2,1)*j]; k1=k1+k2; %#ok<AGROW>
  end
